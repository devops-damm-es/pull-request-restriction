...
test("start_IsAllowedGitPullRequestFalse_ActionResultFalse_Ok", async () => {
    ...
    expect(FakeActionResultWrapperRepositoryService.Message).toBe("Pull request is not allowed: Git pull request between source and target branches is not allowed");
});
